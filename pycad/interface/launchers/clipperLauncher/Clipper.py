...
class Clipper(object):
    ...
    def clip(self, database):
        ...
        # 设置 输入类型 和 返回类型
        libc.clipperEntrance.argtypes = [c_char_p, c_char_p, c_char_p]
        libc.clipperEntrance.restype = c_char_p

        # 调用
        newDatabase = libc.clipperEntrance(configJsonstrForPaths, configJsonstr, databaseJsonstr)
        newDatabase = json.loads(newDatabase)

        # 清除 堆内存 #注意，libc的返回值用完之后，才能执行这一行
        libc.freeHeapMemoryGeneratedByClipper()
...
if __name__ == '__main__':
    ...
    newDatabase = clipper.clip(database)
