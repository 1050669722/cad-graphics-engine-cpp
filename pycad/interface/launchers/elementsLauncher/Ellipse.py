...
class Ellipse(object):
    def __init__(self, ellipseInfoDict):
        # depoly
        copyToDeploy() #TODO: 调试专用，实装注释

        # ellipseInfoDict对象 转换为 jsonstr
        self.ellipseInfoDictJsonstr = json.dumps(ellipseInfoDict) #, ensure_ascii=False
    ...
    def _instantiate(self):
        ...
        # 获取 编码后的jsonstrs
        ellipseInfoDictJsonstr = encodeJsonstr(self.ellipseInfoDictJsonstr)

        # 设置 输入类型 和 返回类型
        libc.ellipseEntrance.argtype = c_char_p
        libc.ellipseEntrance.restype = c_char_p

        # 调用 获取elementIns #TODO: 真正的elementIns的实例不会从这里以jsonstr的形式被loads出来，这里loads出来的还是字典
        elementIns = json.loads(libc.ellipseEntrance(ellipseInfoDictJsonstr))

        # 清除 堆内存 #注意，libc的返回值用完之后，才能执行这一行
        libc.freeHeapMemoryGeneratedByEllipse()

        # 重整属性 #TODO: 所以在这里将elementIns中的信息重整为python中Ellipse类的属性
...
if __name__ == '__main__':
    ...
    try:
        ...
        try:
            ...
            infos = json.load(f)
            f.close()
    ellipseInfoDict = infos["blocks"][0]["ellipses"][0]

    # get coord
    ellipse = Ellipse(ellipseInfoDict)
    # print(ellipse.coord_x)
    # print(ellipse.coord_y)
    pass
