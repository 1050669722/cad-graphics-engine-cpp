...
class Spline(object):
    def __init__(self, splineInfoDict):
        # depoly
        copyToDeploy() #TODO: 调试专用，实装注释

        # splineInfoDict对象 转换为 jsonstr
        self.splineInfoDictJsonstr = json.dumps(splineInfoDict) #, ensure_ascii=False
    ...
    def _instantiate(self):
        ...
        # 获取 编码后的jsonstrs
        splineInfoDictJsonstr = encodeJsonstr(self.splineInfoDictJsonstr)

        # 设置 输入类型 和 返回类型
        libc.splineEntrance.argtype = c_char_p
        libc.splineEntrance.restype = c_char_p

        # 调用 获取elementIns #TODO: 真正的elementIns的实例不会从这里以jsonstr的形式被loads出来，这里loads出来的还是字典
        elementIns = json.loads(libc.splineEntrance(splineInfoDictJsonstr))

        # 清除 堆内存 #注意，libc的返回值用完之后，才能执行这一行
        libc.freeHeapMemoryGeneratedBySpline()

        # 重整属性 #TODO: 所以在这里将elementIns中的信息重整为python中Spline类的属性
...
if __name__ == '__main__':
    ...
    try:
        ...
        try:
            ...
            infos = json.load(f)
            f.close()
    splineInfoDict = infos["blocks"][0]["splines"][0]

    # get coord
    spline = Spline(splineInfoDict)
    # print(spline.coord_x)
    # print(spline.coord_y)
    pass
